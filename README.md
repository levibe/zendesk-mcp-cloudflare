...
One other kind of client is turned away, for an unrelated reason. The server validates the `Origin` header when a request carries one, and accepts only localhost by default, so a browser-based client on any other origin gets a 403. Nothing described on this page is affected — a connector is fetched by Anthropic's servers and `mcp-remote` runs under Node, and neither sends an `Origin` — but it is the first thing to check if a browser client fails with a message about origins rather than about transports.
