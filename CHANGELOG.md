...
- New write tools: update halves for macros, users, groups and organizations, and create-and-update pairs for triggers, automations, Help Center articles and views (#21, #22, #23, #12). All writes go through a registration allowlist; of these, only the macro pair is published to clients.
...
- Migrated to the stateless 2026-07-28 MCP revision: a fresh server per request, no Durable Object, `/sse` removed (#40). Pre-revision clients still work through the legacy path, but a client that can only speak HTTP+SSE cannot connect, and browser requests are now origin-checked (localhost and the worker's own hostname pass by default).
...
- The writes 0.1.0 exposed to every client (ticket create, update and delete, plus user, group and organization creation) are no longer published: registration now withholds any tool not on an explicit allowlist, and only the macro writes are on it. The tools remain defined; publishing one is a one-line, deliberate change.
...
- OAuth state is encoded through UTF-8, so an authorization request carrying non-ASCII text signs in instead of being refused on every attempt (#74). Old-format states and approval cookies keep decoding, permanently, since the cookie lives for a year.
...
- Malformed input to the OAuth endpoints answers a fixed 400 instead of a bare 500, and the pagination cap is enforced rather than merely described (#64).
