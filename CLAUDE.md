...
This is not a style preference, and hoisting the server to module scope to save rebuilding the tools is the mistake to know about. Nothing stops you: `Server.connect` replaces its transport without complaint, so a shared instance answers sequential requests correctly and looks fine locally. It fails only under concurrency, where each new `connect` takes the transport away from an exchange still in flight. Every request but the newest is left with nowhere to send its response and hangs, while the newest returns normally — so the endpoint never looks broken, it just strands traffic behind it. The SDK's single-use check sits on the transport, not the server, so it never fires to warn you.
