...
- The backoff ladder is jittered across the lower half of each step, so the first retry lands somewhere in 500-1000ms rather than on the same millisecond for everyone. This matters because every `GET` retries, so requests that failed together come back together and a fixed ladder would return them in one burst. Half the step rather than the whole of it, so the spread cannot undo the backoff. Do not jitter `Retry-After` to match — jitter spreads a number this client chose, and half the callers would end up asking earlier than Zendesk agreed to. This used to be justified by `get_help_center_hierarchy` fanning out a hundred reads through nested `Promise.all`; that walk is bounded to five in flight and forty per call now, so read the reason as separate callers waking on the same tick rather than one tool flooding
