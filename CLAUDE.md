...
- The backoff ladder is jittered, and `Retry-After` gets a flat spread that lands after the wait it names, never before it. Jitter spreads a number this client chose; moving one Zendesk gave us earlier would have callers asking sooner than it agreed to, so the spread only ever adds. How wide either window is and what not to size it against are decisions about the expressions themselves, so they sit on them in `requestWithRetry` rather than here
