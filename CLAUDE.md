...
- The backoff ladder is jittered across the lower half of each step, so the first retry lands somewhere in 500-1000ms rather than on the same millisecond for everyone. This matters because reads fan out: `get_help_center_hierarchy` issues one per category and then one per section through nested `Promise.all`, so a 503 fails a hundred of them together and a fixed ladder would bring all hundred back in one burst. Half the step rather than the whole of it, so the spread cannot undo the backoff. Do not jitter `Retry-After` to match — jitter spreads a number this client chose, and half the callers would end up asking earlier than Zendesk agreed to
